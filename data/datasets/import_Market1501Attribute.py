...
def import_Market1501Attribute_binary(dataset_dir):
	...
	label.insert(3, 'old')
	print('Re-ordering for group attribute attention learning!...')
	order = [0,1,2,3,12,10,11,4,5,7,9,13,14,15,16,17,18,19,20,6,8,21,22,23,24,25,26,27,28,29]
	label = [label[o] for o in order]
	for id in train_market_attr:
		train_market_attr[id] = [train_market_attr[id][o] for o in order]
	for id in test_market_attr:
		test_market_attr[id] = [test_market_attr[id][o] for o in order]
