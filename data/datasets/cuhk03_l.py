...
class CUHK03_L(BaseImageDataset):
	"""
        cuhk03-np labeled
	"""
	...
	def _process_probe(self, test_list, root):
		cam = np.load('/mnt/lustre/liuyuan1/cvpr20/data/cuhk03-np/labeled/cam_probe.npy')
		dataset = []
	...
	def _process_gallery(self, test_list, root):
		cam = np.load('/mnt/lustre/liuyuan1/cvpr20/data/cuhk03-np/labeled/cam_gallery.npy')
		dataset = []
		with open(test_list, 'r') as f:
			lines = f.readlines()
			for i, l in enumerate(lines):
